...
- `transcriber/config.py`: CLI arguments, presets, prompts, glossary parsing, and `RunConfig`.
- `transcriber/subtitles.py`: subtitle segmentation, confidence cleanup, speaker smoothing, and SRT rendering.
- `transcriber/translation.py`: Spanish-to-English model loading, contextual translation, and glossary handling.
- `transcriber/jobs.py`: durable worker job records, recovery, retention, and queue admission.
- `transcriber/server.py`: authenticated HTTP API and worker execution.
- `transcriber/utils.py`: shared path, timestamp, and bounded log-reading helpers.
- `transcriber/__main__.py`: model execution, file orchestration, watch mode, and the CLI entry point.
...
Install the server and development extras, then run the same checks used for this repository:

```powershell
pip install -e ".[server,dev]"
black --check transcriber tests
ruff check transcriber tests
python -m pytest
```

The test suite also runs through the standard library runner:
...
### Watch `%USERPROFILE%\OneDrive\recordings`
...
```powershell
transcriber --watch --watch-dir "$env:USERPROFILE\OneDrive\recordings" --lang auto --mode quality
```

### LAN worker server

The optional FastAPI worker accepts authenticated uploads, runs quality-mode transcription, and exposes job status plus SRT/text downloads. Full setup, firewall, API, and troubleshooting details are in [docs/worker-server.md](docs/worker-server.md).
...
Every request requires `X-Transcribe-Proxy-Token`. Defaults are one worker, at most two queued-plus-running jobs, a 10 GB upload limit, 24-hour artifact retention, and cold subprocess isolation. Set `TRANSCRIBE_MAX_PENDING_JOBS` to change admission capacity.

`TRANSCRIBE_WARM_VRAM=true` reuses compatible models between jobs. Warm mode requires `TRANSCRIBE_MAX_WORKERS=1` and runs transcription inside the server process, so the default cold mode remains preferable when native crash isolation matters.

Job metadata is persisted under `TRANSCRIBE_WORK_DIR`. Completed and failed status URLs survive restarts; jobs interrupted by a restart become failed with `WORKER_RESTARTED`.
...
```powershell
transcriber --watch --watch-dir "$env:USERPROFILE\OneDrive\recordings" --settle-seconds 20
```
...
- Warm VRAM mode is opt-in. CLI/watch runs retain compatible models in process; server warm mode also requires one worker and trades native crash isolation for model reuse. Cold mode clears model caches and flushes CUDA memory after each run.
