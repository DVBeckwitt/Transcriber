...
- Dependency status: `uv.lock` is updated to WhisperLiveKit `0.2.21`; `pyproject.toml` still allows `whisperlivekit[translation]>=0.2.20.post1` behind the optional `live` extra. Local file post-translation uses Python standard-library HTTP and does not require an HTTP client extra; automatic server startup expects a `vllm` executable in the active Python environment or `PATH`.
