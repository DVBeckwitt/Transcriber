...
- Dependency status: `uv.lock` is updated to WhisperLiveKit `0.2.21`; `pyproject.toml` still allows `whisperlivekit[translation]>=0.2.20.post1` behind the optional `live` extra. Local file post-translation uses `httpx>=0.28` behind the optional `translation-server` extra.
...
- File English conversion is ready for release. `english_output_mode` is selectable through CLI and interactive settings, supports `off` / `direct` / `post` / `auto`, preserves source Spanish/German SRTs, writes English `.en.srt` plus compatibility `.srt` for successful post-translation, logs the selected mode in status, batches local server translation requests, and rejects non-loopback translation server URLs by default. Explicit `post` fails when the server is missing or translation fails; `auto` warns and keeps source output when post-translation is unavailable.
